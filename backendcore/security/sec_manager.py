...
def is_valid_user(prot_name: str, action: str, user: str, auth_key: str):
    valid = True
    prot = fetch_by_key(prot_name)
    print(f'{prot=}')
    if prot:
        valid = prot.is_valid_user(action, user, auth_key)
    return valid


def is_valid_auth_key(prot_name: str, action: str, auth_key: str):
    user = uqry.fetch_by_auth_key(auth_key)
    print(f'{user=}')
    if user:
        user_id = uqry.get_user_id(user)
        print(f'{user_id=}')
        return is_valid_user(prot_name, action, user_id, auth_key)
    else:
        return False


def is_valid_phrase(prot_name: str, user: str, phrase: str):
    return ftpw.phrase_ok(prot_name, user, phrase)


def is_action_ok(prot_name: str, action: str, user_id: str, phrase: str,
                 auth_key: str = None):
    valid_user = is_valid_auth_key(prot_name, action, auth_key)
    valid_phrase = is_valid_phrase(prot_name, phrase, user_id)
    return valid_user and valid_phrase
